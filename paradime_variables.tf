...
variable "node_pools_json" {
  type        = string
  default     = ""
  description = "JSON object of Paradime env node pools keyed by env label (pool name is the label with hyphens stripped, truncated to 12 chars — AKS Linux pool names are lowercase alphanumeric, max 12). Each entry: {vm_size, disk_size, min_size, max_size} plus optional os_sku / zones. Azure has no 1-vCPU Dps size; Standard_D2ps_v5 is the floor. Empty string means use the built-in Paradime defaults in paradime_pools.tf. Passed as a string because Nuon [vars] cannot express complex types."
